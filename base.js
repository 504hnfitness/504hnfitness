...
const LoadingScreen = ({ mensaje = 'CARGANDO...' }) => (
  <div style={{
    ...
    minHeight:'100vh', background:'var(--black)', gap:16
  }}>
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    body{font-family:'Barlow',sans-serif;background:#f0f0f0;display:flex;justify-content:center;padding:40px 20px;min-height:100vh}
    .r{background:#fff;max-width:500px;width:100%;padding:52px 44px;box-shadow:0 4px 32px rgba(0,0,0,0.12);height:fit-content}
    .logo-row{display:flex;align-items:center;gap:12px;margin-bottom:4px}
    .logo-icon{width:36px;height:36px;background:#0a0a0a;clip-path:polygon(20% 0%,80% 0%,100% 20%,100% 80%,80% 100%,20% 100%,0% 80%,0% 20%);display:flex;align-items:center;justify-content:center;color:#f5c518;font-size:18px}
    .logo-text{font-family:'Bebas Neue',sans-serif;font-size:30px;letter-spacing:.04em;color:#0a0a0a}
    .tagline{font-family:'Barlow Condensed',sans-serif;font-size:10px;font-weight:700;letter-spacing:.22em;text-transform:uppercase;color:#888;margin-bottom:20px}
    ...
    .div{height:3px;background:linear-gradient(90deg,#f5c518,#c9a012 70%,transparent);margin:20px 0}
...
<div class="r">
  ...
  <div class="badge">
    <span class="bl">Recibo N°</span>
    <span class="bn">${receiptNo}</span>
  </div>
  <div class="div"></div>
  ...
  <div class="row"><span class="rl">Membresía válida hasta</span><span class="rv">${validoDisplay}</span></div>
  <div class="row"><span class="rl">Método de pago</span><span class="rv">${form.metodo}</span></div>
  <div class="paid">✓ &nbsp;Pago confirmado</div>
  <div class="tb">
      ...
      <div className="modal-box wide" onClick={e => e.stopPropagation()}>
        ...
        <div className="form-grid" style={{gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:4}}>
          ...
          <div className="field-group" style={{marginBottom:0}}>
            ...
            <input type="date" className="field-input" value={form.validoHasta}
        ...
        <p className="modal-note">Se abrirá una ventana nueva con el recibo listo para imprimir o guardar como PDF.</p>
      </div>
    </div>
  );
};
    ...
    <div className="fade-up">
      ...
      <div className="metrics-grid">
        ...
        <div className="metric-card blue fade-up fade-up-4">
          ...
          <div className="metric-change">En los próximos días</div>
        </div>
      </div>
              ...
              {!loadingEgresos && egresos.map((e, i) => {
                ...
                return (
                  <tr key={e.id}>
                    ...
                    <td style={{color:'var(--red)', fontFamily:"'Bebas Neue',sans-serif", fontSize:18}}>
                      L. {parseFloat(e.monto).toLocaleString('es-HN', {minimumFractionDigits:2})}
                    </td>
                    <td style={{color:'var(--gold)', fontFamily:"'Bebas Neue',sans-serif", fontSize:18}}>
                      L. {acumulado.toLocaleString('es-HN', {minimumFractionDigits:2})}
                    </td>
                    <td>
...
const ClientesPanel = ({ clientes, setClientes }) => {
  ...
  const openEditForm = (client) => {
    setForm({ nombre: client.nombre, plan: client.plan, telefono: client.telefono || '', estado: client.estado, vence: client.vence });
  ...
  const handleSave = async (e) => {
    ...
    if (formMode === 'edit') {
      const { data, error } = await db
        .from('clientes')
        .update({ nombre: form.nombre, plan: form.plan, telefono: form.telefono, estado: form.estado, vence: form.vence })
        .eq('id', editId)
        .select()
        .single();
      setSaving(false);
      ...
      const { data, error } = await db
        .from('clientes')
        .insert([{ nombre: form.nombre, plan: form.plan, telefono: form.telefono, estado: form.estado, vence: form.vence }])
        .select()
        .single();
      setSaving(false);
      if (!error) { setClientes([data, ...clientes]); showToast(`${data.nombre} agregado`); }
  ...
  return (
    <div className="fade-up">
      ...
      {receiptClient && (
        <ReceiptModal client={receiptClient} onClose={() => setReceiptClient(null)} />
      )}
            ...
            <div className="form-grid">
              ...
              <div className="field-group" style={{marginBottom:0}}>
                ...
                <select className="form-select" value={form.plan} onChange={e => setForm({...form, plan: e.target.value})}>
                  <option>Diario</option><option>Mensual</option><option>Trimestral</option><option>Anual</option>
                </select>
              </div>
              <div className="field-group" style={{marginBottom:0}}>
              ...
              <div className="field-group" style={{marginBottom:0}}>
                ...
                <input type="date" className="field-input" value={form.vence}
                  onChange={e => setForm({...form, vence: e.target.value})} required />
              </div>
            </div>
            <div style={{display:'flex', gap:12, marginTop:20}}>
              <button type="submit" className="btn-primary" disabled={saving}>
                {saving ? 'Guardando...' : (formMode === 'edit' ? 'Guardar cambios' : 'Guardar cliente')}
              </button>
                ...
                <tr key={c.id}>
                  ...
                  <td>
                    <div style={{display:'flex', gap:6, flexWrap:'wrap'}}>
                      ...
                      <button className="action-btn" onClick={() => setReceiptClient(c)}
                        title="Generar recibo"
                        style={{color:'var(--gold)', borderColor:'rgba(245,197,24,0.3)'}}>🧾</button>
